...
class MainScreen(Screen):
    ...
    # if first batch of 12.5ms  
    first_batch = BooleanProperty(False) 
    ...
    def __init__(self, **kwargs):
        ...
        Clock.schedule_interval(self.algo, 0.03) # inte
    ...
    # algorithm
    def algo(self,*args):
        # if accept data true run algo else skip
        if(self.accept_data):
            # start work
            # if first batch call getnext 6 times
            if (self.first_batch):
                for x in range(0, 6):
                    #mark time
                ...
                # top off to 6 batches
                for x in range(0, 6):
                    #mark time
                    start_time = time.time() 
                    self.getnext(self, *args)
                    end_time = time.time()
                    print "Exec top off:", end_time-start_time
